...
public class LangManager {
    ...
    private static final long DEFAULT_THROTTLE_MS = 2500L;

    private final ForgeToolsPlugin plugin;
    private final Map<String, String> messages = new HashMap<>();

    // UUID → (messageKey → lastSentTimestamp)
    ...
    /**
     * Returns the translated + color-coded + placeholder-replaced string.
     * args: alternating key, value pairs — e.g. "amount", 20, "current", 80
     */
    ...
    /**
     * Sends a message to a player only if the throttle window has elapsed.
     * Uses DEFAULT_THROTTLE_MS (2500ms).
     */
    public void send(Player player, String key, Object... args) {
        send(player, key, DEFAULT_THROTTLE_MS, args);
    }

    /**
     * Sends a message to a player only if at least {@code cooldownMs} ms have passed
     * since the last time this key was sent to this player.
     */
    public void send(Player player, String key, long cooldownMs, Object... args) {
        UUID uuid = player.getUniqueId();
        long now = System.currentTimeMillis();

        Map<String, Long> playerThrottle = throttleCache.computeIfAbsent(uuid, k -> new HashMap<>());
        Long last = playerThrottle.get(key);

        if (last != null && now - last < cooldownMs) return;

        playerThrottle.put(key, now);
        player.sendMessage(get(key, args));
    }

    /** Clears throttle state for a player (e.g. on disconnect). */
