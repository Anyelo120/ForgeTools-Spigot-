...
public abstract class CustomTool {
    ...
    protected boolean canActivate(Player player, ItemStack item) {
        ...
        if (last != null && now - last < (long) cooldownTicks * 50L) {
            ...
            player.sendMessage(plugin.getLangManager().get("cooldown.active", "seconds", remaining));
            return false;
        }
        if (!player.hasPermission("forgetools.use." + id)) {
            player.sendMessage(plugin.getLangManager().get("general.no_permission"));
            return false;
        }
        int energy = plugin.getEnergyManager().getEnergy(player, item, this);
        if (energy < energyCost) {
            player.sendMessage(plugin.getLangManager().get(
                    "energy.not_enough", "current", energy, "required", energyCost));
            return false;
        }
        return true;
    }
