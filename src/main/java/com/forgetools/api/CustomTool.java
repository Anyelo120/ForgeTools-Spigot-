...
public abstract class CustomTool {
    ...
    protected boolean canActivate(Player player, ItemStack item) {
        if (!player.hasPermission("forgetools.use." + id)) {
            plugin.getToolTelemetry().rejection(id, "no_permission", "canActivate");
        ...
        if (last != null && now - last < (long) cooldownTicks * 50L) {
            plugin.getToolTelemetry().rejection(id, "cooldown", "canActivate");
        ...
        if (energy < energyCost) {
            plugin.getToolTelemetry().rejection(id, "not_enough_energy", "canActivate");
            plugin.getLangManager().send(player, "energy.not_enough", 2000L,
                    "current", energy, "required", energyCost);
            return false;
        }
        plugin.getToolTelemetry().activation(id, "canActivate");
        return true;
    }
    ...
    protected Block resolveReferenceBlock(PlayerInteractEvent event, Player player, int maxDistance, String context) {
        Block reference = BlockUtil.resolveReferenceBlock(event, player, maxDistance);
        if (reference != null) return reference;
        plugin.getToolTelemetry().rejection(id, "no_target_reference", context);
        plugin.getLangManager().send(player, "general.no_target_reference", 1200L);
        return null;
    }
