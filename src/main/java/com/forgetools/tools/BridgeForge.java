...
/**
 * BridgeForge — places blocks under the player's feet while sneaking and moving.
 *
 * Uses a dedicated 1-tick repeating task instead of PlayerMoveEvent to guarantee
 * reliable, low-latency detection regardless of look direction or block boundaries.
 *
 * Position delta threshold: 0.08 blocks horizontal movement per tick → ensures
 * blocks are placed on every real step without firing on micro-jitter.
 */
public class BridgeForge extends CustomTool {
    ...
    private final Map<UUID, double[]> lastPositions  = new HashMap<>(); // [x, z]
    ...
    private void tryPlaceBlock(Player player, ItemStack item) {
        ...
        // Pick the first air block at or below feet level
        Block target = null;
    ...
    // BridgeForge uses its own internal task, NOT PlayerMoveEvent
    @Override
