...
public class ClaimPolisher extends CustomTool {
    ...
    public void onRightClick(PlayerInteractEvent event, Player player, ItemStack item) {
        ...
        if (filter == null) {
            player.sendMessage(ChatColor.RED + "No block filter set. Use "
                    + ChatColor.YELLOW + "/forgetools setfilter <material>" + ChatColor.RED + " first.");
            return;
        }
        ...
        if (targets.isEmpty()) {
            player.sendMessage(ChatColor.YELLOW + "No " + filter.name() + " found in this chunk.");
            return;
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (index[0] >= targets.size()) {
                    player.sendMessage(ChatColor.GREEN + "✔ Claim Polisher removed "
                            + removed[0] + "x " + filter.name() + " from this chunk.");
                    cancel();
                }
            }
