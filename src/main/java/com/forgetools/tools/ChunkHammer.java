...
public class ChunkHammer extends CustomTool {
    ...
    public void onRightClick(PlayerInteractEvent event, Player player, ItemStack item) {
        Block target = resolveReferenceBlock(event, player, radius + 2, "chunk_hammer");
        ...
        if (mode == 0) {
            ...
            handleCubeBreak(player, item, target);
        }
    ...
    private void handleCubeBreak(Player player, ItemStack item, Block target) {
